...
def call_api(method, params, token,timeout=10):
    params.append(("access_token", token))
    url = "https://api.vk.com/method/%s?%s" % (method, urlencode(params))
    #print url
    ansewer = json.loads(urllib2.urlopen(url, timeout=timeout).read())
...
if __name__ == '__main__':
    ...
    print token, uid
    friendsList = call_api("friends.get",
            [
                ('user_id', uid)
            ], token)
    print friendsList

    print call_api("photos.get",
            [
                ('owner_id', friendsList[5]),
                #('owner_id', uid),
                ('album_id', 'profile')
            ], token)
