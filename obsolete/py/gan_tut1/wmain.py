...
def mkGenerator(input, weights):
    l1 = tf.nn.relu(tf.matmul(input, weights['w1'] + weights['b1']))
...
def mkDiscriminator(input, weights):
    ...
    l3 = (tf.matmul(l2,weights['w3']) + weights['b3'])
    return l3
...
if __name__ == '__main__':
    ...
    loss_d  = tf.reduce_mean(-(D1) + (D2))
    loss_g  = -tf.reduce_mean((D2))
    ...
    optstep_d = tf.train.RMSPropOptimizer(learning_rate).minimize(loss_d, var_list=dis_vars.values())
    optstep_g = tf.train.RMSPropOptimizer(learning_rate).minimize(loss_g, var_list=gen_vars.values())
    ...
    for iterInd in xrange(20000):
        ...
        if iterInd < 5:
            n_iters_discr = 10
        for innerIter in xrange(n_iters_discr):
            ...
            for key in dis_vars:
                sess.run(dis_vars[key].assign(sess.run(tf.clip_by_value(dis_vars[key], -val2clip, val2clip))))

            #if innerIter < 10:
            #    print iterInd, innerIter, loss_d_val, sess.run(dis_vars['w1'])
        ...
        if iterInd % 1 == 0 and iterInd > 0:
            curData = sess.run(G, feed_dict = {z: np.random.normal(size=[n_points, 1])})
